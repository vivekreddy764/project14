    ...
    for(i = 0; i < limit - 1; i++)
    {
        ...
        for(j = i + 1; j < limit; j++)
        {
            ...
            if(process_queue[i].arrival_time > process_queue[j].arrival_time)
            {
                ...
                temp = process_queue[i]; process_queue[i] = process_queue[j]; process_queue[j] = temp;
            }
        }
    }
}
...
int main()
{
    ...
    for(i = 0, c = 'A'; i < limit; i++, c++)
    {
        ...
        scanf("%d", &process_queue[i].arrival_time ); printf("Enter Burst Time:\t");
        scanf("%d", &process_queue[i].burst_time); printf("Enter Priority:\t");
        scanf("%d", &process_queue[i].priority); process_queue[i].status = 0;
    ...
    for(time = process_queue[0].arrival_time; time < burst_time;)
    {
        ...
        process_queue[largest].ct = time; process_queue[largest].waiting_time = process_queue[largest].ct -
        ...
        wait_time = wait_time + process_queue[largest].waiting_time; turnaround_time = turnaround_time +
